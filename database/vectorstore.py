...
class Store:
    ...
    def __init__(self):
        ...
        self.unsummarised_vectordb=Chroma(collection_name="transcriptvectordb",embedding_function=self.embeddings,persist_directory='./unsummarised_docs')
        self.summarized_vectordb=Chroma(collection_name="transcriptvectordb",embedding_function=self.embeddings,persist_directory='./summarised_docs')

    def ingesting_raw_docs(self,raw_docs):
        """Ingesting raw_docs"""
        try:
            if raw_docs:
                self.unsummarised_vectordb.aadd_documents(raw_docs)
    ...
    def ingesting_summarized_docs(self,summarized_docs):
        ...
        try:
            if summarized_docs:
                self.summarised_vectordb.aadd_documents(summarized_docs)
...
class Retriever:
    ...
    def raw_retriever(self,k=2):
        ...
        un_summarized_retriever=self.vectorstore.unsummarised_vectordb.as_retriever( search_type="similarity_score_threshold",search_kwargs={"k":k})
    ...
    def summarized_retriever(self,k=2):
        """End result returns retriever"""
        summarized_retriever=self.vectorstore.summarised_vectordb.as_retriever(search_type="similarity_score_threshold",search_kwargs={"k":k})
